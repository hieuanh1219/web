...
export default function PropertyDetailPage() {
  ...
  const sections = [
    ...
    { id: "description", label: "Mô tả" },
    { id: "specs", label: "Chi tiết" },
    { id: "gallery", label: "Hình ảnh" }, // Đổi vị trí Gallery xuống dưới để luồng đọc mượt hơn
    { id: "location", label: "Vị trí" },
  ];
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-700 pb-20 md:pb-32">
      ...
      {/* 2. CONTENT BODY */}
      {/* Mobile margin-top âm ít hơn để đỡ bị che mất ảnh Cover */}
      <div className="max-w-7xl mx-auto px-5 md:px-12 relative -mt-20 md:-mt-40 z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-20">
          
          {/* SIDEBAR (Desktop Only) */}
          <aside className="hidden lg:block lg:col-span-3">
            <div className="sticky top-24 pt-4">
              <nav className="flex flex-col gap-4 text-sm border-l border-slate-200 pl-6">
                {sections.map((s) => (
                  <button
                    key={s.id}
                    onClick={() => scrollToSection(s.id)}
          ...
          {/* MAIN STREAM */}
          <main className="lg:col-span-9 flex flex-col gap-10 md:gap-14 pt-2">
            ...
            <section ref={setSectionRef("overview")}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 pt-6">
                {[
                  ...
                  { label: "Phòng ngủ", val: detail.bedrooms, icon: "🛏️" },
                  { label: "Phòng tắm", val: detail.bathrooms, icon: "🛁" },
                  {
                    label: "Pháp lý",
                    val: detail.features?.find((f) => f.key === "legal")?.value,
                    icon: "📜",
            ...
            <section ref={setSectionRef("description")}>
              <SectionHeader title="Thông tin chi tiết" />
              <div className="prose prose-base md:prose-lg prose-slate max-w-none text-slate-600 leading-7 md:leading-8 font-sans text-justify">
                {detail.description?.split("\n").map((p, i) => (
            ...
            {/* D. GALLERY - THEO YÊU CẦU: LƯỚT BÌNH THƯỜNG */}
            <section ref={setSectionRef("gallery")}>
              <SectionHeader title="Hình ảnh thực tế" />
              <div className="flex flex-col gap-6 md:gap-10">
                {/* Render toàn bộ ảnh, không gom nhóm, mỗi ảnh là một khối lớn để lướt */}
                {detail.images?.map((img, idx) => (
                  <div key={idx} className="w-full group">
                    ...
                    />
                    {img.alt && (
                      <p className="mt-2 text-xs md:text-sm text-slate-400 italic text-center">
                        {img.alt}
                      </p>
                    )}
                  </div>
