    ...
    <Link to={`/properties/${card.slug}`} className="group block h-full">
      <div className="relative flex h-full flex-col bg-white rounded-[24px] overflow-hidden transition-all duration-300 hover:shadow-[0_20px_40px_-5px_rgba(14,32,56,0.15)] hover:-translate-y-1 border border-slate-100">
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
          ...
          {/* Lớp gradient nhẹ ở dưới ảnh để làm nổi bật thông tin nếu cần */}
            ...
            <span
              ...
              ${
                card.transactionType === "SALE"
                  ? "bg-emerald-500/90" // Màu xanh lá cho Bán (tạo sự tin tưởng)
                  : "bg-blue-500/90" // Màu xanh dương cho Thuê
              }`}
            >
          ...
          <button
            className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-full bg-white/20 hover:bg-white text-white hover:text-red-500 backdrop-blur-md border border-white/30 transition-all duration-300 active:scale-90"
            onClick={(e) => {
              e.preventDefault(); // Ngăn chuyển trang khi click tim
              // Logic like
            }}
          >
            <CardIcons.Heart />
          ...
          {/* Giá tiền - Hiển thị ngay trên ảnh ở góc dưới trái (Style hiện đại) */}
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          {/* 3. FOOTER INFO (Thông số) */}
          <div className="flex items-center justify-between text-slate-600">
            {/* Trường hợp Đất/Kho (Không có PN/WC) */}
            ...
            {/* Nút mũi tên sang phải (chỉ hiện khi hover vào card, hoặc luôn hiện nhưng đổi màu) */}
