...
export default function PropertyCard({ propertyId }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        <div className="text-sm font-semibold text-slate-900 line-clamp-2">{card.title}</div>

        <div className="mt-2 flex items-center justify-between">
          <div className="text-base font-bold text-slate-900">{card.displayPrice || "—"}</div>
          <div className="text-xs text-slate-600">{card.area ? `${card.area} m²` : "—"}</div>
        </div>
